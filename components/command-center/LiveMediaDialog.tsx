...
function formatTimestamp(value: string | undefined): string {
  ...
  if (!Number.isFinite(time)) return "Capture time unavailable";
...
export function LiveMediaDialog({ open, onClose, incident, media }: LiveMediaDialogProps) {
  ...
  const [incidentMedia, setIncidentMedia] = useState<{ query: string; result: IncidentMediaResult }>();
  ...
  const incidentQuery = incident
    ...
    : "";
  const activeMedia = incidentMedia?.query === incidentQuery ? incidentMedia.result : media;
  ...
  useEffect(() => {
    ...
    void fetch(`/api/live/media?q=${encodeURIComponent(incidentQuery)}&limit=6`, {
      ...
      .then((result) => setIncidentMedia({ query: incidentQuery, result }))
      .catch(() => undefined);
  ...
  return (
    <div className={styles.backdrop} data-live-media-dialog="open">
      ...
      >
        ...
        <div className={styles.truthBar}>
          ...
          <span><AlertTriangle size={13} /> FOOTAGE LOCATION AND TIME REQUIRE VERIFICATION</span>
        </div>
        ...
        <div className={styles.layout}>
          <main className={styles.stage}>
            <div className={styles.player}>
              {selectedVideo && playback.kind === "direct" ? (
                ...
                <div className={styles.emptyPlayer}>
                  <Film size={38} />
                  <strong>No embeddable footage returned</strong>
                  <span>AEGIS keeps the incident report in view. Publisher and discovery links are available below as explicit fallbacks.</span>
                </div>
              )}
            </div>
            ...
            <div className={styles.mediaDetails}>
              ...
              <div className={styles.time}><Clock3 size={14} /> {formatTimestamp(selectedVideo?.publishedAt ?? incident?.observedAt)}</div>
            </div>

            <p id="live-media-disclaimer" className={styles.disclaimer}>
              {selectedVideo?.provenance?.notice ?? activeMedia?.notice ?? "No video is presented as a verified live camera without publisher, capture-time and location confirmation."}
            </p>
          ...
          <aside className={styles.queue} aria-label="Available source media">
            ...
            {!videos.length ? (
              <div className={styles.noQueue}>No source returned an in-site playable asset for this query.</div>
            ) : null}
